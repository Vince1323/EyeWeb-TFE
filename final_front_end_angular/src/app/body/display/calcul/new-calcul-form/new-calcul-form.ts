...
import { DeviceDetectionService } from "src/app/body/service/DeviceDetectionService";
...
export class NewCalculForm implements OnInit, AfterViewInit {
    ...
    formulas: string[] = ['Pearl', 'Cooke', 'SRKT']; //['Pearl', "Cooke", 'Kane', 'HofferQST', 'EVO', 'Barrett', 'HofferQ', 'Holladay', 'SRKT', 'Haigis'];
    biometries: Biometry[] = [];
    ...
    this.biometryService.getExamById(id, this.idOrga).subscribe({
        next: (biometry) => {
            console.log(" Biometry loaded:", biometry); // log de vérification
            this.selectedBiometry = biometry;
            this.disabledCalcul(); // force la mise à jour du bouton
        }
    });
}
    ...
    calculate(): Calcul {
        console.log("selectedBiometry: ", this.selectedBiometry);
