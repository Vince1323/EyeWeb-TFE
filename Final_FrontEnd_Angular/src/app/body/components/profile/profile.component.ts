...
export class ProfileComponent implements OnInit {
    ...
    updateUser() {
        ...
        if (updatedUser.birthDate && typeof updatedUser.birthDate === 'string') {       
            const dateParts = updatedUser.birthDate.split('-');
            const correctedDate = new Date(Number(dateParts[0]), Number(dateParts[1]) - 1, Number(dateParts[2]), 12); // FORCÉ À MIDI
            updatedUser.birthDate = correctedDate.toISOString().split('T')[0];
          }
          else if (updatedUser.birthDate instanceof Date) {        
            const d = updatedUser.birthDate;
            const corrected = new Date(d.getFullYear(), d.getMonth(), d.getDate(), 12); //  FORCÉ À MIDI
            updatedUser.birthDate = corrected.toISOString().split('T')[0];
          }
