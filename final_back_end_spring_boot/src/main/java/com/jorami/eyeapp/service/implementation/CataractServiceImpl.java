...
public class CataractServiceImpl implements CataractService {
    ...
    @Override
    public List<Exam> getAllExamsByPatientId(Long patientId, List<Long> idOrga, String side, boolean isLight) {
        if (side != null) {
            return examRepository.findAllByPatientIdAndEyeSide(patientId, Enum.EyeSide.valueOf(side));
        }
        return examRepository.findAllByPatientId(patientId);
    }
