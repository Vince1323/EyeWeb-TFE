...
public interface ExamMapper {
    ...
    @Mapping(source = ".", target = "patientId", qualifiedByName = "getPatientIdFromLinks")
    ...
    @Named("getPatientIdFromLinks")
    static Long getPatientIdFromLinks(com.jorami.eyeapp.model.exam.Exam exam) {
        if (exam.getLinks() != null && !exam.getLinks().isEmpty()) {
            return exam.getLinks().get(0).getPatient().getId(); // on prend le premier lien disponible
        }
        return null;
    }
